...
function readCondition() {
  return {
    ...
    travelTime: Number(form.travelTime.value),
    ...
    purposeFree: form.purposeFree.value.trim(),
    budget: Number(form.budget.value),
    budgetLabel: form.budget.selectedOptions[0].textContent
  };
}
...
function createPlan(seed, condition, mode, best) {
  ...
  return {
    ...
    day1: {
      morning: short ? '朝は遅めに出発し、最初の目的地は1か所だけに絞ります。到着後はトイレと飲み物を確保してから散策開始。' : '午前中に出発し、途中休憩を1回入れて現地へ。到着したら家族写真を撮り、メインスポットを短時間で楽しみます。',
      lunch: `昼食は${mealIdea(condition, seed)}。混雑前に入れるよう11時台を目安にします。`,
      afternoon: rainy ? '午後は屋内展示、体験施設、駅近の商業施設を中心にして、天気が回復したら短い散歩を追加します。' : nature ? '午後は公園や湖畔、遊歩道などで体を動かします。小さい子はベンチ休憩、大きい子は体験や観察を入れます。' : '午後はメイン施設をもう1つだけ入れ、疲れが見えたらカフェ休憩へ切り替えます。',
    ...
    rainyAlt: rainy ? '最初から屋内施設を軸にし、雨が弱ければ短い屋外散歩を足す組み立てです。' : `雨なら、${seed.area}周辺の水族館・博物館・道の駅・大型商業施設へ切り替え、屋外散策は写真だけにします。`,
...
function mealIdea(condition, seed) {
  if (/海|寺泊|淡路|丹後|伊勢|上越/.test(seed.area) || condition.purposeFree.includes('海鮮')) return '海鮮丼、焼き魚定食、取り分けしやすい魚料理';
